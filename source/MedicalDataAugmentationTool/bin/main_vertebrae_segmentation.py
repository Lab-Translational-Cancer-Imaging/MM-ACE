...
class MainLoop(MainLoopBase):
    ...
    def getDistances(self, center_points):
        num_stdev = np.full(26, np.nan)
        labels = center_points[:,3].astype(int)
        cp = center_points[:, :3]
        if 28 in labels:
            ind = np.argsort(cp[:, -1])[::-1]
            cp = cp[ind]
            labels = labels[ind]
            labels[labels == 28] = 25
        ...
        for image_id in tqdm(self.image_id_list, desc='Testing'):
            #try:
                ...
                if check == 'True':
                    ...
                    clf = joblib.load('mm_ace_skipped_vert.joblib')
                    prob = clf.predict_proba(iqr_test.reshape(-1, 1))*100
                    loc = np.argmax(prob, axis=1)
                    probability_skip = np.round(prob[:,1], 3)
                    probability_skip[iqr_test==0] = 0
                    threshold = 100
                    threshold_unc = 5
                    # print(probability_skip)
                    if np.any((probability_skip < threshold) & (probability_skip > threshold_unc)):
                        print('\nUncertain about scan\n')
                        locations = np.where((probability_skip > threshold_unc) & (probability_skip < threshold))[0]
                        print(f'locations: {locations}')
                        if os.path.exists(os.path.join(self.base_folder, 'uncertainty.csv')):
                            uncertainty = pd.read_csv(os.path.join(self.base_folder, 'uncertainty.csv'), index_col=0).to_dict('records')
                        else:
                            uncertainty = []
                        for loc in locations:
                            uncertainty.append({
                                'file': self.output_folder_handler.path(image_id + '_seg.nii.gz'),
                                'locations': loc,
                                'score': probability_skip[loc],
                                'review': np.nan})
                        pd.DataFrame(uncertainty).to_csv(os.path.join(self.base_folder, 'uncertainty.csv'))
                    if np.any(probability_skip == threshold):
                        ...
                        if os.path.exists(os.path.join(self.base_folder, 'corrected.csv')):
                            corrected = pd.read_csv(os.path.join(self.base_folder, 'corrected.csv'), index_col=0).to_dict('records')
                        else:
                            corrected = []
                        for loc in locations:
                            corrected.append({
                                'file': self.output_folder_handler.path(image_id + '_seg.nii.gz'),
                                'locations': loc})
                        pd.DataFrame(corrected).to_csv(os.path.join(self.base_folder, 'corrected.csv'))
                        ...
                        for loc in locations:
                            ...
                            if np.any(landmarks[i+3:i+6]=='nan'):
                                # print(landmarks[i:i+3])
                                # print(landmarks[i+6:i+9])
    ...
    def review(self):
        """
        The review function. Corrects the manually checked images.
        """
        print('Reviewing...')
        ...
        df_review = df[df['review']==1]
        df_review = df_review[df_review['file'].str.contains(self.image_folder.split('/')[-2])]

        # iterate over all images
        for image_id in tqdm(self.image_id_list, desc='Correcting'):
            if len(df_review) > 0:
                print('\nFound skipped vertebrae\n')
                locations = df_review['locations'].to_list()
                print(f'locations: {locations}')
                landmarks = pd.read_csv(os.path.join(self.setup_folder, 'vertebrae_localization/landmarks.csv'), header=None, keep_default_na= False).to_numpy()[0]
                valid_landmarks = pd.read_csv(os.path.join(self.setup_folder, 'vertebrae_localization/valid_landmarks.csv'), header=None).to_numpy()[0]
                shift = 0
                ...
                df.loc[df_review.index, 'corrected'] = 1
...
if __name__ == '__main__':
    ...
    with MainLoop(hyperparameters) as loop:
        ...
        if parser_args.review == 'False':
            ...
            restart = loop.test(parser_args.check)
            if restart:
                loop.init_datasets()
